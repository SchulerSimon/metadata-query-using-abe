...
class MSP:
    ...
    def createPolicy(self, policy_string):
        ...
        assert type(policy_string) in [
            bytes, str], 'invalid type for policy_string'
    ...
    def _convert_policy_to_msp(self, subtree, curr_vector):
        ...
        if type == OpType.OR:
            left_list = self._convert_policy_to_msp(
                subtree.getLeft(), curr_vector)
            right_list = self._convert_policy_to_msp(
                subtree.getRight(), curr_vector)
        ...
        if type == OpType.AND:
            ...
            left_vector = curr_vector + [0] * \
                (self.len_longest_row - length) + [1]
            right_vector = [0] * self.len_longest_row + \
                [-1]  # [0]*k creates a vector of k zeroes
            # extendedVector = currVector + [0]*(self.lengthOfLongestRow-length)
            # leftVector = extendedVector + [1]
            # rightVector = extendedVector + [2]  # [0]*k creates a vector of k
            # zeroes
            self.len_longest_row += 1
            left_list = self._convert_policy_to_msp(
                subtree.getLeft(), left_vector)
            right_list = self._convert_policy_to_msp(
                subtree.getRight(), right_vector)
    ...
    def _getCoefficientsDict(self, tree, coeff_list, coeff=1):
        ...
        if tree:
            ...
            if (node == OpType.AND):
                ...
                self._getCoefficientsDict(
                    tree.getLeft(), coeff_list, coeff * this_coeff[1])
                self._getCoefficientsDict(
                    tree.getRight(), coeff_list, coeff * this_coeff[2])
            elif (node == OpType.OR):
                this_coeff = self.recoverCoefficients([1])
                self._getCoefficientsDict(
                    tree.getLeft(), coeff_list, coeff * this_coeff[1])
                self._getCoefficientsDict(
                    tree.getRight(), coeff_list, coeff * this_coeff[1])
